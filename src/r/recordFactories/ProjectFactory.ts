...
export class ProjectFactory extends RecordFactory<RT.project> {
   ...
   */
  refreshRecordsLinkedToScene(sceneIdAndRecord: idAndRecord) {
  ...
  refreshRecordsLinkedToLeadGen(leadGenIdAndRecord: idAndRecord) {
    this.linkAutogeneratedVariable(leadGenIdAndRecord, VariableType.string, LeadGenFieldProperty.var_id)
  }

  refreshRecordsLinkedToElement(elementIdAndRecord: idAndRecord) {
  ...
  linkAutogeneratedVariable(idAndRecord: idAndRecord, variableType: VariableType, linkedPropName: string, nameSuffix?: string) {
   ...
   */
  changeRecordName<N extends RT>(id: number, newName?: string): RecordNode<N> | undefined {
