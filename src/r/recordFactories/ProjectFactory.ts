...
export class ProjectFactory extends RecordFactory<RT.project> {
  ...
  addRecord<T>({record, position, id, dontCycleSubRecordIds, parentIdOrAddress}: {
    ...
    switch (record.type) {
      case RT.scene: {
        this.addMenuAndTourModeRecord(idAndRecord.id);
        break;
      }
      case RT.lead_gen_field: {
        const variableIdAndRecord = this.addVariableOfType(VariableType.string);
        if(variableIdAndRecord) {
          ...
          variableIdAndRecord.record.props.var_default = "";
          //Keep the name of the variable same as the lead gen field
          //Going via changeRecordName to take care of duplicate names
          this.changeRecordName(variableIdAndRecord.id, varNameFromOriginName(record.name));
          //Link lead gen field to the variable id
          (record as RecordNode<RT.lead_gen_field>).props.var_id = variableIdAndRecord.id;
        }
        break;
      }
    }
  ...
  addSceneDeepRecord<N extends RT>({ sceneId, record, position, groupElementId }: { sceneId: number, record: RecordNode<N>, position?: number, groupElementId?: number }): RecordNode<N> | undefined {
    ...
    if (sceneJson !== undefined) {
      const addedRecord = (new SceneFactory(sceneJson)).addRecord({ record, position, parentIdOrAddress: groupElementId });
      if (addedRecord !== undefined) {
        this.addLinkedVariables([addedRecord.record as RecordNode<N>]);
        return addedRecord.record;
      }
    }
    return;
  }
      ...
      if (record?.type === RT.element) {
        switch ((record as RecordNode<RT.element>).props.element_type) {
          case ElementType.media_upload: {
            ...
            if(variableRecord) {
              variableRecord.record.props.var_category = VarCategory.autogenerated;
              variableRecord.record.props.var_track = true;
              variableRecord.record.props.var_default = "";
              //Keep the name of the variable same as the lead gen field
              //Going via changeRecordName to take care of duplicate names
              this.changeRecordName(variableRecord.id, varNameFromOriginName(record.name));
              //Link lead gen field to the variable id
          ...
          case ElementType.embed_scorm: {
            ...
            if(scoreVariableRecord) {
              scoreVariableRecord.record.props.var_category = VarCategory.autogenerated;
              scoreVariableRecord.record.props.var_track = true;
              scoreVariableRecord.record.props.var_default = 0;
              this.changeRecordName(scoreVariableRecord.id, varNameFromOriginName(record.name));
            ...
            if(suspendDataVariableRecord) {
              suspendDataVariableRecord.record.props.var_category = VarCategory.autogenerated;
              suspendDataVariableRecord.record.props.var_track = true;
              suspendDataVariableRecord.record.props.var_default = 0;
              this.changeRecordName(suspendDataVariableRecord.id, varNameFromOriginName(record.name));
            ...
            if(progressVariableRecord) {
              progressVariableRecord.record.props.var_category = VarCategory.autogenerated;
              progressVariableRecord.record.props.var_track = true;
              progressVariableRecord.record.props.var_default = 0;
              this.changeRecordName(progressVariableRecord.id, varNameFromOriginName(record.name));
  ...
  addMenuAndTourModeRecord(sceneId: number) {
    ...
    if(menuRecord) {
      menuRecord.record.props.menu_scene_id = sceneId;
      menuRecord.record.props.menu_show = this.getValueOrDefault(rtp.project.auto_add_new_scene_to_menu);
    }
    ...
    if (this.getValueOrDefault(rtp.project.auto_add_new_scene_to_tour_mode) === true) {
      ...
      if(tourModeRecord) {
        tourModeRecord.record.props.tour_mode_scene_id = sceneId;
      }
    }
  }
